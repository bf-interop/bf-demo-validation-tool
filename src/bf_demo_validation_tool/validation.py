...
def summarize(*args, **kwargs):
    ...
    for key, shape in shapes.items():
        ...
        shacl_summary.appendChild(shape_tr)

    # Change label
    shacl_label = js.document.getElementById("shacl_size")
    shacl_label.innerHTML = (
        f"{len(current_shacl)} triples from {len(shapes)} SHACL Resources"
    )
...
def _add_shape(validation_graph: rdflib.Graph, shape: dict):
    ...
    properties = {}
    for obj in validation_graph.objects(subject=shape["id"], predicate=SHACL.property):
...
async def validate(*args, **kwargs):
    ...
    if len(incoming_graph) > 0:
        ...
        validation_element.appendChild(alert)
    ...
    validation_element.appendChild(row)
